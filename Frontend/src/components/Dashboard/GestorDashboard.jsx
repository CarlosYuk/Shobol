...
const GestorDashboard = () => {
  ...
  const eliminarNotificacion = async (id) => {
    try {
      await fetch(`http://localhost:5000/api/mensajes/${id}`, { method: "DELETE" });
      // Vuelve a consultar los mensajes para reflejar el cambio real
      const mensajesActualizados = await fetch("http://localhost:5000/api/mensajes").then(res => res.json());
      ...
      <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-6 mt-8">
        ...
        <div className="space-y-4">
          {mensajes
            ...
            .map((m) => (
              <div key={m.id} className="bg-amber-100 p-4 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
